...
client_id = '98e6a284-89c1-47eb-a460-c9ce1fd7379a'
...
def get_data(endpoint, parameters):
    r = requests.get(endpoint, parameters, auth=(client_id, 'a7dfaadb-7e7e-47b5-8111-b68f3491370c'))
    # Extract JSON data
    json = r.json()
...
def get_info(df, elements_list):
    ...
    for i, row in df.iterrows():
        ...
        info_df = info_df._append(info_i, ignore_index=True)
        ...
        for print_ii, (s_ii, e_ii) in enumerate(time_range):
            ...
            for d in data:
                ...
                if (missing_val == df_i[df_i.index != 'time']).all():
                    continue
                df = df._append(df_i, ignore_index=True)
            if len(df) == 0:
